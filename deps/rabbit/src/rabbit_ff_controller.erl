...
inventory_rpcs(Nodes, Timeout) ->
    %% The feature flag registry in `rabbit_ff_registry' was implemented with
    %% a module which was dynamically regerated and reloaded in the past. To
    %% avoid deadlocks with the Code server we need to first call into
    %% `rabbit_ff_registry_wrapper' if it is available. If it is unavailable,
    %% we fall back to `rabbit_ff_registry'.
    %%
